...
export async function generateMetadata({ params }: { params: { lang: string, period: string } }): Promise<Metadata> {
  const { period } = params
...
export default async function page({ params }: { params: { lang: string, period: string } }) {
  ...
  return (
    <div className="flex flex-col gap-4 mx-auto py-8">
            <h1 className="text-3xl font-bold mt-6">{t.title}</h1>
            <NavigationBar currentPeriod={legislatur.period} />
            <p className="mt-6 text-foreground/60">{t.elected_members} {legislatur.legislatur_start}</p>
            <Table>
                <TableHeader>
                    <TableRow>
