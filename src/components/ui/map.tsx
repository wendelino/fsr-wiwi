...
import type { RouteData } from "@/utilities/routing";
import { cn } from "@/utilities/ui";

// Get theme from document data-theme attribute
...
function useResolvedTheme(themeProp?: "light" | "dark"): "light" | "dark" {
  ...
  useEffect(() => {
    ...
    const observer = new MutationObserver(() => {
      ...
      if (docTheme) {
        ...
        const systemTheme = getImplicitPreference();
        if (systemTheme) {
    ...
    const handleSystemChange = (e: MediaQueryListEvent) => {
      // Only use system preference if no document theme is set
      if (!(getDocumentTheme() || contextTheme)) {
  ...
  return themeProp ?? contextTheme ?? detectedTheme;
}
...
const Map = forwardRef<MapRef, MapProps>(function Map(
  ...
  useEffect(() => {
    ...
    const initialStyle = mapStyles[resolvedTheme];
  ...
  // Handle style change when theme changes
  useEffect(() => {
    if (!(mapInstance && resolvedTheme)) {
      return;
    }

    const newStyle = mapStyles[resolvedTheme];
...
export function FullMap({
  ...
  return (
    <Map
      ...
      className="h-[66vh] overflow-hidden p-0"
      zoom={14}
    >
      <MapControls
        position="bottom-right"
        ...
        <MarkerPopup>
          <div className="space-y-1">
            ...
            <p className="text-muted-foreground text-xs">
              lat: {draggableMarker.lat.toFixed(6)}, lng:{" "}
